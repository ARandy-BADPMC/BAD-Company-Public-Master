...
if(_loadout != -1) then  
	...
	if (count _nObjects == 0) then {
		...
		if (_vehicle in _attackType) then 
			...
			{
				//if (count allPlayers > 10 || _vehicle in ["RHS_MELB_AH6M","I_Heli_light_03_dynamicLoadout_F"]) then {
				...
				//} else {
				//  hint "This vehicle can only be spawned when there are more than 10 players online.";	
				//};
